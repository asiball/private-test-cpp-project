...
この流れが、ドキュメント（`docs/`）からコード（`driver/` `lib/` `cli/`）、
CI設定（`.github/` `.gitlab-ci.yml`）まで一貫して追えます。
...
**場所**: `lib/include/device.hpp` / `lib/src/device.cpp`

クラスの実装詳細を内部クラス（`Impl`）に隠蔽し、ヘッダには宣言だけを残す。

```cpp
// lib/include/device.hpp — ヘッダには Impl の宣言のみ
class Device {
    struct Impl;                      // 前方宣言だけ
    std::unique_ptr<Impl> impl_;      // ポインタで保持
};

// lib/src/device.cpp — 実装の詳細はここだけ
struct Device::Impl {
    ISpiDriver* driver;
    bool        owns_driver;
    ...
...
**場所**: `driver/include/ispi_driver.hpp`
...
**なぜ重要か**:
- `Device` クラスは `ISpiDriver*` しか知らないため、実機なしでテストできる
...
**場所**: `lib/include/device.hpp` — テスト用コンストラクタ

`Device` クラスのコンストラクタを2種類用意している:
...
```cpp
// tests/unit/lib/test_device.cpp
MockSpiDriver mock;
EXPECT_CALL(mock, transfer(_, _, _)).WillOnce(Return(5));

Device d(&mock);   // 実機不要でテスト可能
```
...
**場所**: `driver/include/version.hpp.in` / `CMakeLists.txt`

```cmake
# CMakeLists.txt: git tag からバージョンを取得
execute_process(
    COMMAND git describe --tags --match "driver/v*" --abbrev=0
    OUTPUT_VARIABLE _tag
)
# version.hpp.in の @EDS_VERSION_DRIVER@ を置換して version.hpp を生成
configure_file(version.hpp.in version.hpp @ONLY)
```

```cpp
// version.hpp — cmake が自動生成する
inline constexpr const char* DRIVER = "driver/v1.0.1";
...
**場所**: `driver/include/ispi_driver.hpp` + `tests/unit/driver/test_spi_driver.cpp`
...
- `SpiDriver` が正しく呼ばれているかを `Device` の上位から検証できる
...
**場所**: `CMakeLists.txt`（`ENABLE_SANITIZERS` オプション）/ `.github/workflows/ci.yml` / `.gitlab-ci.yml`
...
cmake -S tests/unit/lib -B build-asan/test-lib \
    -DCMAKE_BUILD_TYPE=Debug \
    -DCMAKE_CXX_FLAGS="-fsanitize=address,undefined -fno-omit-frame-pointer \
                       -I$(pwd)/lib/include -I$(pwd)/tests/mocks" \
    -DCMAKE_EXE_LINKER_FLAGS="-fsanitize=address,undefined"
cmake --build build-asan/test-lib
ASAN_OPTIONS=detect_leaks=1 ./build-asan/test-lib/test_device
```
...
cmake -S tests/unit/lib -B build-tsan/test-lib \
    -DCMAKE_BUILD_TYPE=Debug \
    -DCMAKE_CXX_FLAGS="-fsanitize=thread -fno-omit-frame-pointer \
                       -I$(pwd)/lib/include -I$(pwd)/tests/mocks" \
    -DCMAKE_EXE_LINKER_FLAGS="-fsanitize=thread"
cmake --build build-tsan/test-lib
TSAN_OPTIONS=halt_on_error=1 ./build-tsan/test-lib/test_device
```

#### このプロジェクトでの学習ポイント: `read_async()` のスレッド危険性

`lib/src/device.cpp` の `read_async()` を見てほしい:
...
このコードは `Device` オブジェクトのライフタイムが「コールバック完了まで保証される」前提で動作する。
呼び出し側が `Device` を先に破棄した場合、use-after-free（ASANが検出）またはデータ競合（TSANが検出）が発生しうる。
...
GitHub Actions より詳細なパイプラインで、**差分ビルド**（変更されたコンポーネントのみビルド）、
**clang-tidy**、**pandocによるPDF生成**、**リリースパッケージ作成**を含む。

**合わせて読む**: [GitLab移行・運用ガイド](../gitlab-migration-guide.md)
