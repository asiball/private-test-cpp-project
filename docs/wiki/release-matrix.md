...
|---|---|---|---|---|---|
| v1.0.0 | v1.1.0 | v1.1.0 | `spi-hal/v1.0.0` は EAGAIN リトライ未対応のため、高負荷環境で SPI 転送が失敗しやすい（[v1.1.0 リリースノート](../deliverables/07_delivery/release-notes/v1.1.0.md) の後方互換性節を参照） | 2025-04-10 | spi-hal/v1.0.1 で修正済み |
...
> タグ体系（`release.yml` / `sbom.yml` のトリガ）は `i2c-hal` / `gpio` / `libadxl345` / `libmcp9808`
> を含む **7 コンポーネント**を対象とするため、これらが初リリースされたら列・節を追加する。
