# C++設計パターンガイド — embedded-device-suite

| 項目 | 内容 |
|---|---|
| 対象読者 | C++による組み込みSW開発の実践的な設計パターンを学びたいエンジニア |
| 前提知識 | C++の基本文法、オブジェクト指向設計の基本概念 |
| 学習時間目安 | 1〜2時間 |
...
クラスの実実装詳細を内部クラス（`Impl`）に隠蔽し、ヘッダには宣言だけを残す。
...
コンパイラに `-fsanitize=address,undefined`（ASAN+UBSAN）または `-fsanitize=thread`（TSAN）フラグを渡すと、バイナリにランタイム検査コードが挿入されます。
