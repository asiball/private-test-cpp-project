...
> **本プロジェクトでは `miscdevice` を採用**：`cdev_init` + `device_create` の組み合わせより
> 行数が少なく、`/dev/my_spi_dev` の自動生成も任せられる。実装は
> `kernel/my_spi_driver.c` の `misc_register(&priv->misc)` を参照。
...
ioctl(fd, MY_SPI_IOC_TRANSFER, &xfer);   // 実コードは spi-hal/src/kernel_spi_driver.cpp
```
