...
`Sensor::read_raw_async()` は内部で `std::thread([...]() { ... }).detach()` する。
Sensor オブジェクトがコールバック完了前に破棄されると、スレッドが解放済みメモリを参照することになる。

**ASan**: コールバック実行時に use-after-free として検出
**TSan**: 破棄スレッドとコールバックスレッドが Sensor のメンバ変数に同期なしでアクセスしていれば、データ競合として検出

このような非自明な並行バグが、人手のテストでは再現しにくくても、
サニタイザーは決定論的に検出してくれる。
