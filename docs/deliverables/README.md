...
（図が表示されない環境向けのテキスト版）
- **利用者・発注者** → **A層**（API/IF/CLI 仕様：04, 05）を読む。
- **保守・実装者** → **A層**で全体を把握し、必要に応じて **B層**（詳細設計：03）へ深掘りする。
- **開発者（リファレンス引き）** → **C層**（Doxygen HTML：自動生成）で全 API を引く。
...
| **libmcp9808** ★任意 | ライブラリ（実装・単体テスト完備） | ⬜ 未作成（仕様書は演習 #48 の範囲外） | ⬜ 未作成 | ⬜ 未作成（MCP9808 データシート参照） |
...
| **examples** | 使用例デモ | — | — | 納品文書対象外（各コンポーネントの使い方を示すサンプル）|
| **rust** | 参照実装 | — | — | 納品文書対象外（Rust リライト探求。[rust-migration-guide](../guides/rust-migration-guide.md) 参照）|
...
| Word (.docx) | pandoc + `tools/docs-reference.docx`（無ければ §5.2 のとおり自動生成。リポジトリには未コミット）| `output/docx/` |
