 ...
 *
 * - 分解能: 0.0625 °C/LSB（符号込み 13bit: 符号1 + 整数8 + 小数4）
...
public:
     ...
     * @brief デストラクタ
     *
     * 自前生成した内部ドライバ（path 指定コンストラクタの場合）は所有権を持つ
     * `unique_ptr` の解放に伴い RAII で close される。テスト用コンストラクタで
     * 注入したドライバは所有しないため close されない（呼び出し元の責務）。
     */
    ~Mcp9808();
